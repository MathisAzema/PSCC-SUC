...
function thermal_unit_commit_constraints2(model, instance)
    """
    Define first-stage constraints in the 3-bin formulation
    """
    is_on=model[:is_on]
    ...
    @constraint(model, thermal_fixed_cost>=sum(unit.ConstTerm*is_on[unit.name, t]+unit.StartUpCost*start_up[unit.name, t]+unit.StartDownCost*start_down[unit.name, t] for unit in thermal_units for t in 1:T)+sum(unit.LinearTerm*power_ref[unit.name, t] for unit in thermal_units for t in 1:T))
