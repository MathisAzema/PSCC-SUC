function add_cut_SP(cb_data, options::Options, master_pb::JuMP.Model, oracle_pb, instance::Instance, Time_subproblem, solution_x::Vector{Matrix{Float64}}; force::Float64, S::Int64, batch=1, gap, ρ=0.0)
    ...
    if thermal_fixed_cost_val+dispatch_first_stage_val+sum(cut_parameters[s].objective_value[t] for s in 1:S for t in 1:T)/S >= (1+0.01*gap/100)*(thermal_fixed_cost_val+dispatch_first_stage_val+thermal_cost_val)
        options._add_optimality_cuts(cb_data, master_pb, instance, cut_parameters; S=S)
    end

    return thermal_fixed_cost_val+dispatch_first_stage_val+sum(cut_parameters[s].objective_value[t] for t in 1:T for s in 1:S)/S, [cut_parameters[s].objective_value for s in 1:S]
end


function _add_optimality_cuts_SP(cb_data, master_pb, instance::Instance, cut_parameters::Vector{oracleResults}; S::Int64)
    T= instance.TimeHorizon
